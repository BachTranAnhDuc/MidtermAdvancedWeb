...
app.get("/getListUser",async (req,res) => {
    ...
    return res.json({success:true, msg: "Get list user success", listUser: data})
})
...
app.post("/login", async (req, res) => {
    ...
    console.log(user)
    if(user === null || user === undefined) return res.json({success:false, msg:"This user is not exists"})
...
app.post("/addNewStudent", async (req, res) =>{
    ...
    if(isExist) return res.json({success:false, msg:"Create student fail this student is exists"})
    student.create({
        id: data.id,
        name: data.name,
        major: data.major,
...
app.delete("/deleteStudent/:id", async (req,res) => {
    ...
    if(isExist === undefined || isExist === null)
        return res.json({success: false, msg:`Delete student id ${id} fail, not exist in database`})
    isExist.deleteOne()
...
app.put("/updateStudent/:id", async (req, res) => {
    ...
    const isExist = await student.findOne({id: id})
    if(isExist === undefined || isExist === null)
        return res.json({success: false, msg:`Update student id ${id} fail, not exist in database`})
...
app.get('/getDetails/:id', async (req, res) => {
    ...
    const isExist = await student.findOne({id: id})
    if(isExist === undefined || isExist === null)
        return res.json({success: false, msg:`Get student id ${id} fail, not exist in database`})
    return res.json({success: true, msg:`Get student id ${id} success`, data: isExist})    
})
